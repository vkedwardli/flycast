    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                ID3D11SamplerState* sampler = tex->pointSampling ? bd->pPointSampler : bd->pTextureSampler;
...
static void ImGui_ImplDX11_DestroyTexture(ImTextureData* tex)
{
    ...
    if (backend_tex == nullptr)
        return;
    IM_ASSERT(backend_tex->pTextureView == ((ImTextureDX11*)tex->TexID)->shaderResourceView);
    if (backend_tex->pTextureView) { backend_tex->pTextureView->Release(); backend_tex->pTextureView = nullptr; }
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        // Store identifiers
        backend_tex->PublicTex.shaderResourceView = backend_tex->pTextureView;
