...
void gui_loadFonts()
{
	...
	// gdxsv: Japanese text is always needed. Meiryo UI and MS UI Gothic ship with every Windows install.
	{
		FontEntry entry; entry.lang = "ja"; entry.paths = { fontDir + "meiryo.ttc" }; entry.fontNo = 2; entry.size = fontSize * 0.95f;
		registerFont(fonts, entry);
	}
	{
		FontEntry entry; entry.lang = "ja"; entry.paths = { fontDir + "meiryob.ttc" }; entry.fontNo = 2; entry.size = fontSize * 0.95f;
		registerFont(boldFonts, entry);
	}
	{
		FontEntry entry; entry.lang = "ja"; entry.paths = { fontDir + "msgothic.ttc" }; entry.fontNo = 2; entry.size = fontSize * 0.95f;
		registerFont({ &fonts, &boldFonts }, entry);
	}
